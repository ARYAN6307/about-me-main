...
export interface SmartImageProps extends React.ComponentProps<typeof Flex> {
  ...
  enlarge?: boolean;
  src: string | { src: string; title: string; description: string };  // Update type to allow both string and object
  unoptimized?: boolean;
  sizes?: string;
  priority?: boolean;
}
...
const SmartImage: React.FC<SmartImageProps> = ({
  ...
  // Handle src as an object or a string
  const imageUrl = typeof src === 'object' && src.src ? src.src : src;  // Extract the URL from the object if needed
  const imageTitle = typeof src === 'object' && src.title ? src.title : "";  // Extract title if available
  const imageDescription = typeof src === 'object' && src.description ? src.description : "";  // Extract description if available
  ...
  const isVideo = typeof imageUrl === 'string' && imageUrl.endsWith(".mp4");
  const isYouTube = isYouTubeVideo(typeof imageUrl === "string" ? imageUrl : "");
      ...
      >
        ...
        {!isLoading && isYouTube && (
          <iframe
            width="100%"
            height="100%"
            src={getYouTubeEmbedUrl(typeof imageUrl === "string" ? imageUrl : "")}
            frameBorder="0"
